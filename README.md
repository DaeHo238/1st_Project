...
│   ├── dashboard.html  # 핵심 판정·수치를 한 화면에 요약한 대시보드
...
10. **종합 결론, 한계, 비즈니스 인사이트** — 접근성 지표별 판정(무엇이 실제 신호이고 무엇이 검출편향인지), 데이터·분석 양쪽의 한계 구분과 이를 넘으려면 어떤 데이터가 필요한지(종단 패널·검사 기반 결과지표), 시장 기회·제품 실현 가능성·피해야 할 기획·KPI 설계 리스크
